...
class NMAPEEvaluator:
    ...
    Forecast input may come from CSV files (``forecast_files``) or in-memory
    DataFrames (``forecast_frames``), but not both. When using
    ``forecast_frames``, each DataFrame must contain ``basis_time``,
    ``forecast_time`` and ``mu`` columns. Use ``to_nmape_frames`` in the
    adapter module to build these from a ForecastResult.
    ...
    def __init__(
        ...
        *,
        forecast_files: Optional[Dict[int, Union[str, Path]]] = None,
        forecast_frames: Optional[Dict[int, pd.DataFrame]] = None,
        ...
        forecast_frames: Dictionary mapping forecast horizon (int) to a
                        DataFrame with basis_time, forecast_time, mu columns.
                        Mutually exclusive with forecast_files.
        column_mapping: Dictionary to override default column names.
        """
        if (forecast_files is None) == (forecast_frames is None):
            raise ValueError(
                "Provide exactly one of 'forecast_files' or 'forecast_frames'."
            )
    ...
    def _read_single_forecast(self, horizon: int, fp: Union[str, Path]):
    ...
    def _frames_to_forecast_df(self) -> Optional[pd.DataFrame]:
        req_cols = [
            self.cols["basis_time_col"],
            self.cols["forecast_time_col"],
            self.cols["forecast_mu_col"],
        ]
        parts = []
        if self.forecast_frames is not None: 
            for horizon, df in self.forecast_frames.items():
                missing = [c for c in req_cols if c not in df.columns]
                if missing:
                    raise KeyError(
                        f"Required columns missing in forecast_frames[{horizon}]. "
                        f"Missing: {missing}"
                    )
                part = df[req_cols].copy()
                part[self.cols["basis_time_col"]] = pd.to_datetime(
                    part[self.cols["basis_time_col"]]
                )
                part[self.cols["forecast_time_col"]] = pd.to_datetime(
                    part[self.cols["forecast_time_col"]]
                )
                part["horizon"] = horizon
                parts.append(part)
            return pd.concat(parts, ignore_index=True)
        else: 
            raise ValueError("No forecast frames provided.")

    def _load_data(self):
        print("Loading forecast data...")

        if self.forecast_frames is not None:
            self.forecast_df = self._frames_to_forecast_df()
        elif self.forecast_files is not None:
            all_forecasts = []
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [
                    executor.submit(self._read_single_forecast, horizon, fp)
                    for horizon, fp in self.forecast_files.items()
                ]
            ...
            if not all_forecasts:
                raise ValueError(
                    "No valid forecast data could be loaded. Please check the 'forecast_files' dictionary paths and file contents.")

            self.forecast_df = pd.concat(all_forecasts, ignore_index=True)
        else: 
            raise ValueError("Either 'forecast_frames' or 'forecast_files' must be provided.")
